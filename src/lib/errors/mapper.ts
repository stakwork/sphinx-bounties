...
import { ValidationError, NotFoundError, DatabaseError, ConflictError } from "./classes";
...
export function mapErrorToAppError(error: unknown): AppError {
  ...
  if (error instanceof Error) {
    return new AppError("UNKNOWN_ERROR", error.message, 500, false, {
      originalError: error.name,
      stack: error.stack,
    });
  }

  return new AppError("UNKNOWN_ERROR", "An unknown error occurred", 500, false, {
    error: String(error),
  });
}
