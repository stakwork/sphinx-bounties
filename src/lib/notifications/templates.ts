...
export const crudToasts = {
  createSuccess: (resource: string) => toast.success(`${resource} created successfully`),
  createError: (resource: string) =>
    toast.error(`Failed to create ${resource}`, "Please try again"),
  updateSuccess: (resource: string) => toast.success(`${resource} updated successfully`),
...
export const formToasts = {
  validationError: () => toast.error("Validation failed", "Please check the form for errors"),
...
export const bountyToasts = {
  ...
  assigned: (hunterName: string) => toast.success(`Bounty assigned to ${hunterName}`),
  ...
  paymentSent: (amount: string) => toast.success("Payment sent", `${amount} sats transferred`),
};
...
export const workspaceToasts = {
  ...
  memberAdded: (name: string) => toast.success(`${name} added to workspace`),
  memberRemoved: (name: string) => toast.success(`${name} removed from workspace`),
  roleUpdated: (name: string, role: string) => toast.success(`${name} role updated to ${role}`),
  budgetUpdated: () => toast.success("Budget updated successfully"),
  inviteSent: (email: string) => toast.success("Invite sent", `Invitation sent to ${email}`),
};
